$(window).ready(function() {
  ...
  $('[data-toggle="offcanvas"]').click(function () {
    ...
    //manage active and submenu in main menu
    $( '.header .navbar-nav a' ).on( 'click', function () {
	$( '.header .navbar-nav' ).find( 'li.active' ).removeClass( 'active' );
	$( this ).parent( 'li' ).addClass( 'active' );
        $('#submenu ul.nav.navbar-nav').addClass('collapse');
        $(this).parent('li').find('ul.nav.navbar-nav.collapse').removeClass('collapse');
    });
    ...
    $(function () {
        $('#phMaster_FiltroPedidos_ctl00_txtFechasPedido_pnlPickerDesde').datetimepicker({
            locale: 'es'
        });
    });
